...
//            $this->logger->debug($stmt->queryString);
    ...
    private function generateRecordInsertStatement(Record $record, &$nameTypeMap): string {
        //$recordName = MySQLDictionarySchemaGenerator::quoteString(strtolower($this->labelDictionnaire)."_".strtolower($record->getName()));
        ...
        try {
            if(count($values) > 2100 ){
                ...
                foreach($chunkedValues as $chunkedValue){
                    ...
                    $count_with_chunk = $this->targetConnection->executeStatement($stm2, $chunkedValue);
                    $count += $count_with_chunk;
                }
            }else{
                $count = $this->targetConnection->executeStatement($stm, $values);
            }
    ...
    public function serializeCases(): int {
        ...
        try {
            ...
            $strMsg .= "[ JE SUIS ICI: " . count($values) . ', ' . count(array_fill(0, count($this->casesMap), $singlePlaceholder)) . ', '.$length_chunk. ',' .count($chunkedValues). ', ' . $limited . ',' .count($chunkedValue);
            $this->logger->error($strMsg, array("context" => (string) $e));
            throw new \Exception($strMsg, 0, $e);
        }
        return $count;
    ...
    public function serializeRecord(Record $record) {
        ...
        try {
            if(count($values) > 2100 ){
                $colonnes = floor(count($values)/count(array_fill(0, $recordCount, $singlePlaceholder)));
                if(1200 % $colonnes != 0){
                    $length_chunk = floor(1200 / $colonnes) * $colonnes;
                }else{
                    $length_chunk = 1200;
                ...
                foreach($chunkedValues as $chunkedValue){
                    $limited = (count($chunkedValue)/$colonnes);
                    $part_fill = array_slice(array_fill(0, $recordCount, $singlePlaceholder), 0, $limited);
                    $stm2 = $this->generateRecordInsertStatement($record, $nameTypeMap);
                    $placeholders2 = implode(', ', $part_fill);
            ...
            $strMsg .= "[ JE SUIS ICI: " . count($values) . ', ' . count(array_fill(0, $recordCount, $singlePlaceholder)) . ', '.$length_chunk. ',' .count($chunkedValues). ', ' . implode(',', $chunkedValue) . ',' .count($chunkedValue);
            $this->logger->error($strMsg, array("context" => (string) $e));
            throw new \Exception($strMsg, 0, $e);
        }
    }
